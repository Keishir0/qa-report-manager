...
1. Alinhamento com o Status Geral do Relatório:
   - Se o Status Geral (ou o relato/instruções) for "Aprovado QA": Todos os passos devem ser marcados com status "Aprovado QA", e o resultado obtido ('actualResult') de cada um deve descrever detalhadamente a confirmação do sucesso observado (nunca use "Pendente de execução" ou frases genéricas).
   - Se o Status Geral (ou o relato/instruções) for "Reprovado QA":
     * Os passos anteriores ao erro/bug devem ser marcados com status "Aprovado QA" (sucesso), e o resultado obtido ('actualResult') deve detalhar o sucesso dessas ações.
     * O passo específico onde ocorre o erro/falha deve ser marcado com status "Reprovado QA", e o resultado obtido ('actualResult') deve descrever detalhadamente o erro observado (mensagens de erro, quebras de layout, falhas de funcionamento).
...
3. 'actualResult':
   - Se o status for "Aprovado QA": Descreva detalhadamente a confirmação do sucesso (ex: "O redirecionamento ocorreu com sucesso e a tela anterior foi exibida perfeitamente"). Nunca deixe em branco ou genérico.
   - Se o status for "Reprovado QA": Descreva detalhadamente o erro ou quebra observada no relato (ex: "O reprodutor de áudio quebrou o layout da tela e a reprodução falhou").
...
Regras de campos:
1. Se a branch não for identificada, use "alfa".
...
5. O 'generalStatus' do relatório deve ser determinado estritamente a partir do status dos passos gerados:
   - Se todos os passos gerados tiverem status "Aprovado QA", o 'generalStatus' deve ser "Aprovado QA".
   - Se algum passo gerado tiver status "Reprovado QA", o 'generalStatus' deve ser "Reprovado QA".
   - Se não houver "Reprovado QA", mas houver pelo menos um "Não Executado", o 'generalStatus' deve ser "Não Executado".
...
corretamente, use generalStatus "Aprovado QA" e passos com status "Aprovado QA".
`;
